...
Application related activities can be found under `AuditLogs_CL`, filtering by `Category == ApplicationManagement`. By filtering by the IP addresses previously known by the threat actor shown in the Defender alert, we were able to get the first action performed by the user.
